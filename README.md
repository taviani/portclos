...
Optional checkboxes on **Run workflow**:

- **submit_ios** — iOS preview build + TestFlight (`EXPO_ASC_APP_ID` required)
- **submit_android** — Android preview build + Play **internal** (`releaseStatus: completed` in `eas.json`; Google Play service account must already be linked in EAS)

Neither checked: iOS build only, no submit. Both checked: queue iOS + Android with submit (`--no-wait`).
...
5. GitHub secret `EXPO_ASC_APP_ID` for iOS TestFlight when **submit_ios** is checked.
